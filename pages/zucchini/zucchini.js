    ...
    .then((mainHtml) => {
      ...
      function getMealFilters() {
        const checked = document.querySelectorAll(
          '.meal-checkbox[type="checkbox"]:checked'
        );
        selectedMealTypes = Array.from(checked).map((x) => x.value);
        let stringOfSelectedMealTypes = "";
        if (selectedMealTypes.length > 0) {
          stringOfSelectedMealTypes =
            "&mealType=" + selectedMealTypes.join("&mealType=");
        }
      ...
      function getIngredientsFilters() {
        const checked = document.querySelectorAll(
          '.ingredients-checkbox[type="checkbox"]:checked'
        );
        ...
        if (recipesObject.count === 0) {
          ...
          recipesObject.hits.forEach((recipe) => {
            ...
            imageFigure.addEventListener("click", () =>
              window.location.replace(recipe.recipe.url)
            );
